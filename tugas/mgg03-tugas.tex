...
\begin{bclogo}[couleur=ctpBase, arrondi=0.1, logo=\bccrayon, ombre=false, couleurBord=ctpSurface, epBord=0.6]{Perhatikan hal berikut:}
  \begin{coloredPen}
    ...
    \item Seluruh eksperimen memakai \textbf{inferensi} pada ResNet50 pra-latih --- jangan melakukan \code{fit()}, \code{backward()}, atau pelatihan ulang classifier.
...
\begin{itemize}[itemsep=2pt,parsep=0pt,topsep=2pt,partopsep=2pt]
  ...
  \item[\color{ctpBlue}\faFileCode] \textbf{Paket wajib:} \code{torch}, \code{torchvision}, \code{Pillow}, \code{matplotlib}, \code{scikit-learn}.
...
\begin{bclogo}[couleur=ctpMantle, arrondi=0.1, logo=\bcinfo, ombre=false, couleurBord=ctpSurface, epBord=0.6]{Batasan Cakupan}
  ...
  \code{fit()}, \code{backward()}, atau pelatihan ulang classifier. Fokus tugas adalah
  mengamati dan menganalisis representasi visual.
\end{bclogo}
\end{minipage}
...
% ------------------------------------------------------------
\clearpage
\subsection{Soal 2 --- ResNet50 sebagai Feature Extractor}
\label{sec:soal2}
...
% ------------------------------------------------------------
\subsection{Soal 3 --- Kemiripan Kosinus Antar-Gambar}
\label{sec:soal3}
...
% ------------------------------------------------------------
\clearpage
\subsection{Soal 4 --- Visualisasi Embedding Gambar}
\label{sec:soal4}
...
\textbf{(b)} Ekstrak embedding setiap gambar menggunakan pipeline pada Soal 2.
...
% ------------------------------------------------------------
\subsection{Soal 5 --- Analisis Kritis Representasi Visual}
\label{sec:soal5}
